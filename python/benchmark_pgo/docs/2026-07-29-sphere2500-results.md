...
> **更正（2026-08-01）：本文的方法排序没有迁移到真实数据。**
>
> 下文「GM 损失不可用」「默认损失定为 TLS」的结论只在 sphere2500 上成立。真实
> 序列的全序列实验给出相反的排序：TLS 的硬截断会把子图之间的约束整片切掉、把地图
> 拆散，GM 的软降权反而稳定，管线最终默认是 `gnc_gm`。
>
> 原因是两边的残差归一化尺度不可比：合成图的边噪声是标定过的，真实管线的 loop 边
> sigma 是 `loop_sigma / conf` 这个启发式（本文「barc² 阈值选取」一节已经指出过这个
> 差异，只是当时没意识到它足以翻转方法排序）。本基准的作用因此限于接入前的冒烟
> 验证——确认求解器跑得通、有 outlier 时能收敛——**不作为方法选择的依据**。
...
**这一节的结论已被真实数据推翻**（见文首更正）：GM 在合成图上「权重停在判据之上、
等价于没有剔除」的失效模式，在真实序列里恰恰是它比 TLS 稳的原因——真实 loop 边的
sigma 是启发式的，TLS 的硬截断会连同真实约束一起切掉。最终默认是 `gnc_gm`。
