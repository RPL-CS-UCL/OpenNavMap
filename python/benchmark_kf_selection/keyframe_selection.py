...
def pre_compute(scene_path, str_matcher, str_estimator):
	...
	if timestamps is None:
		...
		group_size = total_images // num_groups
		remainder = total_images % num_groups

		submap_data = []
		start_idx = 0		
		for group_idx in range(num_groups):
			current_group_size = group_size + 1 if group_idx < remainder else group_size
			end_idx = start_idx + current_group_size
			frames = sorted(image_names[start_idx:end_idx])
			
			start_time = group_idx * Time_Threshold
			end_time = (group_idx + 1) * Time_Threshold
			submap_data.append((start_time, end_time, np.array(frames)))
			start_idx = end_idx

		timestamps = {img_name: 0 for img_name in poses.keys()}
	else:
	...
	if not os.path.exists(os.path.join(scene_path, 'iqa.txt')):
	...
	if not os.path.exists(os.path.join(scene_path, 'descriptors.txt')):
	...
	###################################
	##### NOTE(gogojjh): the exhausive computation among pairs is slow, skip it now
