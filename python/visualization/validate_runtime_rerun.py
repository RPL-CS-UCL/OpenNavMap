...
def validate_runtime_rerun(event_dir: Path, render_trace: Path, rrd: Path) -> Dict[str, object]:
    ...
    summary["passed"] = all(
        [
            ...
            summary["keyframe_transform_coverage"] == f"{len(expected_transforms)} / {len(expected_transforms)}",
            summary["keyframe_pinhole_coverage"] == f"{len(expected_pinholes)} / {len(expected_pinholes)}",
            summary["keyframe_image_coverage"] == f"{len(expected_images)} / {len(expected_images)}",
            summary["current_keyframe_image_rendered"],
            summary["odom_edges_rendered"] == f"{len(expected_edge_paths['odom'])} / {len(expected_edge_paths['odom'])}",
            summary["covis_edges_rendered"] == f"{len(expected_edge_paths['covis'])} / {len(expected_edge_paths['covis'])}",
            summary["trav_edges_rendered"] == f"{len(expected_edge_paths['trav'])} / {len(expected_edge_paths['trav'])}",
