...
这不是一个“单纯的 LiteVLoc 定位仓库”。

更准确地说，当前仓库本质上承载的是 **OpenNavMap** 系统：一个面向 **multi-session mapping** 的 topometric map 系统，目标是从多次采集的数据中构建、对齐、合并并维护可用于导航的轻量级拓扑度量地图。

其中，**LiteVLoc** 是这个系统中的一个核心子模块，职责是：
...
- `python/image_graph.py` / `python/image_node.py`
  - 维护带图像、descriptor、相机内参、深度等信息的共视图
  - 用于地图关键帧表示，也是 LiteVLoc 使用的地图观测层

- `python/point_graph.py` / `python/point_node.py`
  - 维护 odometry graph 与 traversability graph
  - 面向位姿链、通行性建图和最短路径规划

- `python/utils/base_graph.py` / `python/utils/base_node.py`
  - 图与节点的基础抽象层
...
当前仓库中的 `third_party/` 目录承载了 OpenNavMap 所依赖的重要外部模型组件。
...
- **当前仓库**：OpenNavMap 的核心地图、定位、规划与融合逻辑
...
3. `python/image_graph.py`
4. `python/point_graph.py`
5. `python/loc_pipeline.py`
6. `python/global_planner.py`
7. `python/pose_fusion.py`
...
**OpenNavMap 是主系统，目标是构建和维护 multi-session topometric map；LiteVLoc 是其视觉定位子模块，负责基于该地图执行 global visual localization，并进一步支撑导航与位姿融合。**
